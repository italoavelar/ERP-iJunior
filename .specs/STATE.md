...
- **Acceptance journeys**: A gerente (login → shell → plano → parcelas → ativação → receipt parcial → refresh), B vice-presidente (timeline → reversal → saldo/auditoria), C usuário sem FinanceCapability (shell sem Financeiro e API 403), D `PLATFORM_ADMIN` sem bypass, E sessão/logout/revogação, F retry idempotente, G history freeze após reversal e H overdue restaurado após reversal foram cobertas por testes reais e de integração.
- **Security review**: nenhum P0/P1 aberto; actor spoofing, privilege bypass, CSRF/Origin, vazamento de credencial/token, mass assignment, BigInt cru, mutação de ledger/audit e corridas financeiras foram verificados e/ou corrigidos.
...
- **Known limitations / future**: rate limiting de login permanece uma preocupação de infraestrutura futura (não prevista na spec aprovada); a implantação deve aplicar proteção de borda e injetar o `ContractReferencePort` Comercial real antes de produção. Nenhum novo módulo ou ciclo de negócio foi iniciado neste bloco.
...
- **Technical decision (T04–T38)**: PostgreSQL efêmero isolado aplica migrations Prisma do zero para integration/API tests; mutações usam capability no use case, preflight e advisory lock de idempotência, `PaymentPlan FOR UPDATE`, ledger/auditoria/idempotência no mesmo commit; Hono recebe contexto autenticado por adapter e expõe apenas DTOs fechados e rotas de comando explícitas.
...
- **Principal files (T20–T38)**: `apps/api/src/modules/finance-contract-receivables/{application,http,infrastructure}/**`, `apps/api/src/app.ts`, `scripts/postgres-test-harness.ts`, `package.json` e `.specs/features/finance-contract-receivables/tasks.md`.
