...
export function LoginPage() {
  ...
  return <main className="login-page"><form className="login-card" onSubmit={submit}><div><p className="muted">Plataforma compartilhada</p><h1>Entrar no ERP iJúnior</h1><p className="muted">Acesse seu espaço de trabalho com segurança.</p></div><label className="field">Email<Input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" /></label><label className="field">Senha<Input required type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" /></label>{error ? <p className="field-error" role="alert">{error}</p> : null}<Button disabled={busy}>{busy ? "Entrando…" : "Entrar"}</Button></form></main>;
}
